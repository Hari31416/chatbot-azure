...
* **Resolution**: 
  ...
  * *Result*: Names are now guaranteed to be globally unique and secure against resource name collisions.
...
* **Issue**: The `Azure for Students` subscription has a strict region restriction policy assignment (`sys.regionrestriction`) that limits resource deployments to the following regions: `eastasia`, `centralindia`, `malaysiawest`, `koreacentral`, `uaenorth`.
  * *Conflict*: Our initial Static Web App fallback region of `eastus2` triggered a policy violation and blocked the entire template deployment.
...
* **Resolution**:
  ...
  * *Result*: The Key Vault name is now globally unique, resolving the DNS naming conflict while adhering to the 24-character maximum constraint.
